...
from test_integracion_caretakers import (
    test_crear_caretaker_exitoso,
    test_crear_caretaker_email_duplicado,
    test_get_all_caretakers,
    test_get_caretaker_by_id,
    test_get_caretaker_by_id_invalido
)
...
def run_tests():
    test_functions = {
        "Crear caretaker exitoso": test_crear_caretaker_exitoso,
        "Crear caretaker con email duplicado": test_crear_caretaker_email_duplicado,
        "Obtener todos los caretakers": test_get_all_caretakers,
        "Obtener caretaker por ID": test_get_caretaker_by_id,
        "Obtener caretaker por ID inválido": test_get_caretaker_by_id_invalido
    }
...
def obtener_nombre_pdf_incremental(base_name='reporte_integracion_tests_caretakers'):
...
def generar_pdf(resultados, path_pdf):
    ...
    story.append(Paragraph(
        "Resultados de prueba del backend-registro-actividades para los endpoints de caretakers",
        styles['Title']
    ))
    story.append(Spacer(1, 12))
