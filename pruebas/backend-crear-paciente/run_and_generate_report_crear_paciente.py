...
from test_integracion_crear_paciente import (
    test_crear_paciente_exitoso,
    test_crear_paciente_email_duplicado,
    test_get_all_pacientes,
    test_get_paciente_by_id,
    test_get_paciente_by_id_invalido
)
...
def run_tests():
    test_functions = {
        ...
        "Crear paciente con email duplicado": test_crear_paciente_email_duplicado,
        "Obtener todos los pacientes": test_get_all_pacientes,
        "Obtener paciente por ID": test_get_paciente_by_id,
        "Obtener paciente por ID inválido": test_get_paciente_by_id_invalido
    }
...
def obtener_nombre_pdf_incremental(base_name='reporte_integracion_tests_pacientes'):
...
def generar_pdf(resultados, path_pdf):
    ...
    story.append(Paragraph(
        "Resultados de prueba del backend-registro-actividades para los endpoints de pacientes",
        styles['Title']
    ))
    story.append(Spacer(1, 12))
...
if __name__ == '__main__':
    # Suprimir la salida estándar y errores
    sys.stdout = open(os.devnull, 'w')
    sys.stderr = open(os.devnull, 'w')
    
    resultados = run_tests()

    # Restaurar la salida estándar y errores
